...
describe('Testing JSON', () => {
	...
	test('Testing JSON.stringify', () => {
		const data = { key: "Hello World", nested: [1, 2, { deep: true }] };

		expect(JSON.stringify(data)).toBe('{"key":"Hello World","nested":[1,2,{"deep":true}]}');
	});

	test('Testing JSON.parse', () => {
		const data = '{"key":"Hello World","nested":[1,2,{"deep":true}]}'

		expect(JSON.parse(data)).toStrictEqual({ key: "Hello World", nested: [1, 2, { deep: true }] });
	});
});